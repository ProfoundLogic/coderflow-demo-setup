...
Use `"field type": "spinner"` on an item to render this widget.

Properties below are **specific to `spinner`** (plus any other widgets they also apply to). For properties that apply to every widget (id, position, font, borders, padding, events, validation, etc.) see [_universal.md](_universal.md).
...
Defines the minimum value for a spinner or slider element.

### `max value`

**Default:** `blank` · **Format:** `number`

Defines the maximum value for a spinner or slider element.

### `increment value`

**Default:** `1` · **Format:** `number`

Specifies how much the value in a spinner or slider element increases or decreases.
...
Initiates a client-side script when the up or down arrow is clicked on a spinner element.
