...
Use `"field type": "file upload dnd"` on an item to render this widget.

Properties below are **specific to `file upload dnd`** (plus any other widgets they also apply to). For properties that apply to every widget (id, position, font, borders, padding, events, validation, etc.) see [_universal.md](_universal.md).
...
After dropping a file, should a page submit happen automatically. Upload happens during submit.
...
When true, show the "Select Files" link in addition to "Drop files here" text.
