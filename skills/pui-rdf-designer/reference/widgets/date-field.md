...
Use `"field type": "date field"` on an item to render this widget.

Properties below are **specific to `date field`** (plus any other widgets they also apply to). For properties that apply to every widget (id, position, font, borders, padding, events, validation, etc.) see [_universal.md](_universal.md).

Specific property count: **38** · Combined with universal: **128**
## Field Settings
...
**Choices:** `true`, `false`, `auto-select`

Controls whether the user has a Today option on the calendar pop-up to jump to the current date.

The special value 'auto-select' causes the Today option to place the current date value into the date field and close the pop-up automatically.
...
Displays column with the week number within a time period. Default is 52 (1 year). See pui.calcWeekNum to customise

### `calendar position`

**Default:** `[blank]` · **Bind data types:** `char`, `varchar`, `string`

**Choices:** `below-right`, `above-right`, `right`, `below-left`, `above-left`, `left`

Controls the position and orientation of the calendar. The default [blank] is the same as 'right'
